...
const Projects = () => {
  return (
    ...
    >
      ...
      <div className="absolute bottom-20 left-10 w-32 h-32 rounded-full bg-purple-100/50 dark:bg-purple-900/20 blur-3xl -z-10"></div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        >
          ...
          <h2 className="text-4xl lg:text-5xl font-extrabold text-slate-900 dark:text-slate-100 mb-3">
        ...
        >
          {projectsData.map((project, index) => (
            <motion.div
              ...
              className={cn(
                "rounded-3xl overflow-hidden shadow-xl border transition-all duration-300",
                "bg-white dark:bg-slate-800/50 border-slate-100/70 dark:border-slate-700/50", 
                "md:flex", 
                "group", 
                index % 2 === 1 ? 'md:flex-row-reverse' : '', 
                 "hover:shadow-2xl hover:border-indigo-100/50 dark:hover:border-indigo-700/70" 
              )}
              whileHover={{ y: -5 }} 
            >
              <div className="md:w-2/5 relative aspect-video md:aspect-auto h-full min-h-[300px]">
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/10 opacity-30 dark:opacity-20"></div> {/* Dark mode gradient overlay */}
                <img
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-start p-6">
                  <motion.a
                    ...
                    className="inline-flex items-center gap-2 text-white font-semibold bg-gradient-to-r from-indigo-600 to-purple-600 px-5 py-2.5 rounded-lg shadow-md transform transition-all hover:scale-105 hover:shadow-lg"
                    whileHover={{ y: -2 }}
                  >
              ...
              <div className="p-8 md:w-3/5 flex flex-col">
                <div className="flex items-start gap-4 mb-4">
                  <div className="p-2.5 bg-indigo-100/50 dark:bg-indigo-900/40 rounded-lg text-indigo-500 dark:text-indigo-400"> 
                    {project.icon}
                  </div>
                ...
                <div className="mb-6">
                  <h4 className="font-semibold text-slate-700 dark:text-slate-200 mb-3 text-base flex items-center gap-2"> 
                    <CheckCircle className="text-indigo-500 dark:text-indigo-400" size={18} /> 
                    Key Features
                  </h4>
                  <ul className="space-y-3 text-slate-600 dark:text-slate-300"> 
                    {project.details.map((detail, idx) => (
                      <motion.li
                        ...
                        animate="visible"
                      >
                        <div className="w-2 h-2 rounded-full bg-indigo-500 dark:bg-indigo-400 mt-2 flex-shrink-0"></div> 
                        <span>{detail}</span>
                      </motion.li>
                    ))}
                  </ul>
                ...
                <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-700"> 
                  ...
                  <div className="flex items-center gap-5">
                    <motion.a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 transition-all duration-200 group" 
                      whileHover={{ x: 2 }}
                    >
                      <ExternalLink size={16} className="transition-transform group-hover:rotate-[-3deg]" />
                      Live Demo
                      <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-all duration-200 ml-0.5" /> 
                    </motion.a>
                    {project.githubLink && (
                      <motion.a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition-all duration-200 group" 
                        whileHover={{ x: 2 }}
                      >
